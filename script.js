document.addEventListener('DOMContentLoaded', function() {
    ...
    function getPokemonId(pokemonName) {
        // Your existing getPokemonId function here
        switch (pokemonName) {
    ...
    customOptionList.forEach(option => {
        option.addEventListener('click', function() {
            ...
            if (selectTrigger && realSelect) {
                ...
                realSelect.dispatchEvent(new Event('change')); // Trigger the native select's change event
...
incrementButton.addEventListener('click', () => {
    const selectedPokemon = realSelect.value; // Use the value of the real select
...
function loadProgress() {
    ...
    if (savedPokemon !== null && savedPokemon !== 'none' && realSelect) {
        currentPokemon = savedPokemon;
        realSelect.value = savedPokemon;
        // Update the custom trigger text on load
        const selectedOption = realSelect.querySelector(`option[value="${savedPokemon}"]`);
        if (selectedOption && selectTrigger && selectTrigger.querySelector('span')) {
            selectTrigger.querySelector('span').textContent = selectedOption.textContent;
        }
    } else {
        currentPokemon = '';
        if (realSelect && selectTrigger && selectTrigger.querySelector('span')) {
            realSelect.value = 'none';
            selectTrigger.querySelector('span').textContent = 'Select a Pokémon';
        }
    }
